...
FOR ALL USING (
  group_meeting_id IN (
    SELECT gm.id FROM public.group_meetings gm 
    WHERE gm.discipler_id = public.get_current_user_profile_id()
  )
);
...
CREATE POLICY "Authenticated users can update meeting photos" 
